...
namespace CGL {
    void MeshEdit::init() {
        smoothShading = false;
        shadingMode = false;
        shaderProgID = loadShaders("shader/vert", "shader/frag.glsl");
        if (!shaderProgID)
            shaderProgID = loadShaders("../shader/vert", "../shader/frag.glsl");
    ...
    void MeshEdit::update_camera() {
        ...
        glUniform1f(glGetUniformLocation(shaderProgID, "time"), (float) ms / 1000);
        glUniform1i(glGetUniformLocation(shaderProgID, "itime"), (GLint) t % 10);
        glUseProgram(0);
    }
